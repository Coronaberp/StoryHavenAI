...
async def list_lora_training_jobs(current_user: dict = Depends(require_capability("lora_training.view", "View training jobs and their checkpoints."))):
    return await lora_training_repo.list_jobs()

@api.get("/admin/lora-training/checkpoints")
async def list_local_checkpoints(current_user: dict = Depends(require_capability("lora_training.view", "View training jobs and their checkpoints."))):
    try:
...
async def delete_lora_training_job(jid: str, current_user: dict = Depends(require_capability("lora_training.manage", "Delete a job or checkpoint, or abort a running job."))):
...
async def list_job_checkpoints(jid: str, current_user: dict = Depends(require_capability("lora_training.view", "View training jobs and their checkpoints."))):
    return await lora_training_repo.list_checkpoints(jid)

@api.delete("/admin/lora-training/checkpoints/{cid}")
async def delete_job_checkpoint(cid: str, current_user: dict = Depends(require_capability("lora_training.manage", "Delete a job or checkpoint, or abort a running job."))):
...
async def request_lora_checkpoint(jid: str, current_user: dict = Depends(require_capability("lora_training.view", "View training jobs and their checkpoints."))):
    job = await lora_training_repo.get_job(jid)
    if not job:
        raise HTTPException(404, "not found")
...
async def abort_lora_training_job(jid: str, current_user: dict = Depends(require_capability("lora_training.manage", "Delete a job or checkpoint, or abort a running job."))):
    job = await lora_training_repo.get_job(jid)
    if not job:
        raise HTTPException(404, "not found")
...
async def create_and_stream_lora_training_job(
        ...
        current_user: dict = Depends(require_capability("lora_training.start", "Start a new LoRA training job.")),
