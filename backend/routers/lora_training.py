...
async def create_and_stream_lora_training_job(
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for i, img in enumerate(images):
            ...
            zf.writestr(f"img_{i:04d}{ext}", data)
            zf.writestr(f"img_{i:04d}.txt", body.trigger_word)
    images_zip = buf.getvalue()
    ...
    task = asyncio.create_task(_execute_training_job(
        job, config, images_zip, current_user, architecture,
...
async def _execute_training_job(job: dict, config: dict, images_zip: bytes, current_user: dict, architecture: str,
    ...
    try:
        ...
        for label, cache_name, cache_path in to_cache:
            await lora_training_repo.update_job(
                job["id"], log=f"Checking Modal's model cache for {label} ({cache_name})…")
            await modal_client.ensure_model_cached(cache_name, cache_path,
    ...
    modal_events = modal_client.stream_training_job(config, images_zip)
    ...
    try:
        ...
        async for event in modal_events:
            ...
            if etype == "error":
                await lora_training_repo.update_job(job["id"], status="failed", error=event.get("message", ""))
                await notification_repo.notify_admins("admin_lora_training", f"LoRA training failed: {job['name']}",
                                       event.get("message", ""), "/images/training", related_id=job["id"])
                return
            if etype == "progress":
        ...
        if not saw_terminal_event:
            ...
            # exception raised) without ever sending a final "done" or
            # "error" SSE event — happened for real once already: the GPU
            # function fully finished training on Modal's side (confirmed
            # via `modal app logs`/`modal container list` — 0 active
            # containers, no errors) but this loop's `async for` just
            # silently ran out of events, leaving the job frozen at
            # status="training" forever with nothing to explain why.
            msg = "Training stream ended without a final result — check the Modal dashboard's App Logs " \
            ...
            await lora_training_repo.update_job(job["id"], status="failed", error=msg)
            await notification_repo.notify_admins("admin_lora_training", f"LoRA training failed: {job['name']}",
                                   msg, "/images/training", related_id=job["id"])
    except modal_client.ModalNotConfigured as e:
        await lora_training_repo.update_job(job["id"], status="failed", error=str(e))
        await notification_repo.notify_admins("admin_lora_training", f"LoRA training failed: {job['name']}",
                               str(e), "/images/training", related_id=job["id"])
    except Exception as e:
        log.warning("lora_training: job failed job=%s: %s: %s", job["id"], type(e).__name__, e)
        await lora_training_repo.update_job(job["id"], status="failed", error=str(e))
        await notification_repo.notify_admins("admin_lora_training", f"LoRA training failed: {job['name']}",
                               str(e), "/images/training", related_id=job["id"])
    finally:
