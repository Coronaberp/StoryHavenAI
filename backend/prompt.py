...
def build_system(char, persona, user_name, mode="character", language="English", full=True):
    ...
    if mode == "rpg":
        ...
        if persona and persona.get("description"):
            parts.append(_untrusted(f"# Player character — {persona['name']}", sub(persona["description"])))
        if full:
            parts.append(sub(RPG_IMMERSION_PROMPT))
            parts.append(DICE_RPG)
            if char.get("dialogue"):
                parts.append(_untrusted("# Prose style reference", sub(char["dialogue"])))
        else:
        ...
        if persona and persona.get("description"):
            parts.append(_untrusted(f"# {persona['name']} (who you're with)", sub(persona["description"])))
        if full:
            parts.append(
                "# How to play\n"
            ...
            if char.get("dialogue"):
                parts.append(_untrusted("# Tone & style reference", sub(char["dialogue"])))
        else:
            parts.append(
                "# Rule reminder\n"
