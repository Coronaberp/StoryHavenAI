...
def build_extract_prompt(transcript: str, char_name: str, user_name: str, language: str,
    ...
    return (
        ...
        "A fact must reflect something the story actually confirmed or showed happening, not "
        "something merely mentioned, asked about, rumored, or hedged (heard, apparently, might be, "
        "supposedly). A secondhand or hedged remark about someone who has not appeared in this "
        "exchange is not confirmed information about them — either leave it out, or if it is worth "
        "keeping, record it as what was said or rumored rather than as a settled trait of that "
        "person.\n"
...
def build_lore_update_prompt(drafts: list[FactDraft], lore_neighbors: list[list[dict]]) -> str:
    ...
    return (
        ...
        "Never override a lore entry's core identity (a character's name, species, occupation, "
        "role, or allegiance) on the strength of a fact that is itself hedged, secondhand, or not "
        "something the story has actually confirmed happened on-screen. Overriding core identity "
        "requires a real, witnessed event or a direct statement in the story, never a rumor, a "
        "passing remark from someone else, or a description of a character who never appeared in "
        "the scene. Example: a NEW fact that only reports a rumor about a character who was not "
        "present in the scene (\"heard she's apparently a bit strange\") must NOT override that "
        "character's established occupation or role in the NEARBY lore entry — that is not a "
        "genuine contradiction, it is hearsay about someone unconfirmed.\n\n"
        "NEW facts:\n" + "\n".join(new_lines) + "\n\n"
        "NEARBY lore entries:\n" + "\n".join(neighbor_lines) + "\n\n"
