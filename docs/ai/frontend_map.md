...
`new_ui/css/themes.css` + `new_ui/js/theme.js`: 6 accent presets (Aurum/Azure/Crimson/Verdant/Amethyst/Rose Quartz) × 2 chrome bases (dark/light) = 12 combinations, faithfully porting `legacy_ui/js/appearance.js`'s actual algorithm (not just static brand swatches — the old color-reference file `storyhaven_brand_template_visual.html` was deleted; `themes.css` itself is now the canonical color source). `theme.js` only ever toggles `data-theme`/`data-accent` attributes on `<html>` and persists the choice via `store`; it holds no color values itself. The temporary top-right icon button (sun/moon, `cycleTheme()`) stands in for a future Settings UI that only needs to set/persist those same two attributes.
...
2. **Light mode's default accent is its own muted tone, not the dark-mode hex.** `--color-accent-base` is `#E3BD6C` in dark but `#A8791C` in light (`--color-accent-deep-base` similarly `#F2CE87`/`#7C5A14`) — a custom preset's raw hex gets blended 85%/15% against *this mode-appropriate anchor* (`color-mix(in srgb, {accent} 85%, var(--color-accent-base) 15%)`), which is what keeps a bright preset color from washing out unreadable against the pale `#FAF7F0` light paper.
...
Every `new_ui/js/*.js` file is a classic script sharing one global scope, so two files defining the same function name silently overwrite each other by load order. This has already shipped twice — `_pickerTileHtml` was defined in both `imagegen-pickers.js` and `admin-train-picker.js`, which broke every model preview tile and made clicks no-op. Before naming a new top-level function, grep for the name across `new_ui/js/`. See `architecture.md`.
