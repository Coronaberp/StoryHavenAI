...
4. **Embed v3** (spec written) — component-based share-card rework (per-content-type composition instead of one shared layout forced onto everything), artwork-forward, shrunk brand watermark, gold reserved for accents only, real SVG icons (no emoji), real data only on every stat (several of the original mockup's numbers — a star rating, "chapter 14," per-user "Worlds"/"Stories" — had no backing data and were dropped or replaced). Character/Profile split out of the shared `_compose_profile_card` composer, Group/Shared-chat split out of the shared `_compose_group_card` composer, both in `server.py`. Docs card (`docs-og.png`) goes from a static pre-rendered image to dynamic rendering — not auth-aware, since Discord's link-unfurl crawler fetches the image with no user session, so the PNG itself can never reflect the eventual viewer's login state; it always shows the same locked/teaser treatment.
