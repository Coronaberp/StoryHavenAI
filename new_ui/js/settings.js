...
class SettingsView {
  ...
  render() {
    ...
    this.main.innerHTML = `
      ...
      ${this.testSiteDomain ? settingsRowHtml({ icon: svgIcon("beaker"), label: t("settings_row_test_site", "Dev test server (early access, unstable)"), sublabel: t("settings_row_test_site_sub", "Try the newest features before they're stable. Separate account, can go down anytime."), right: `<button type="button" onclick="settingsView.toggleTestSiteAccess()" class="px-3 py-1.5 rounded-md text-xs font-medium flex-none" style="${ME?.test_site_warning_acknowledged ? "border:1px solid var(--color-line-2);color:var(--color-sec)" : "background:var(--color-accent);color:var(--color-paper)"}">${ME?.test_site_warning_acknowledged ? t("settings_test_site_hide_button", "Hide") : t("settings_test_site_unlock_button", "Unlock")}</button>` }) : ""}
