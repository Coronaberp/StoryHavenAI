...
class ExploreCreatorsView {
  ...
  async load() {
    ...
    try {
      ...
      this.creators = await api(`/api/users${qs ? `?${qs}` : ""}`);
    } catch (err) {
      this.error = err.message || t("creators_couldnt_load_creators");
      this.creators = [];
    }
    this.loading = false;
    this.render();
  ...
  mountCustomCards() {
    this.main.querySelectorAll("[data-card-user]").forEach((box) => {
      const a = this.creators.find((x) => x.username === box.dataset.cardUser);
      if (!a) return;
  ...
  rowHtml(a) {
    if (a.card_html && a.card_html.trim()) {
      return `<div class="creator-card creator-card-custom" data-card-user="${_attr(a.username)}" style="cursor:pointer" onclick="navigate('/u/${encodeURIComponent(a.username)}')"></div>`;
    }
    ...
    return `
      <div class="creator-card creator-card-compact" data-username="${_attr(a.username)}" style="cursor:pointer" onclick="navigate('/u/${encodeURIComponent(a.username)}')">
        <div class="creator-banner" style="${banner}"></div>
        ...
        <div class="creator-body">
          ...
          ${!isSelf && ME ? `
            <button type="button" class="filter-chip${a.following ? " on" : ""}" data-follow-btn data-feature="follows"
              ...
              <span data-follow-label>${a.following ? t("profile_following", "Following") : t("creators_follow", "Follow")}</span>
            </button>
          ` : ""}
        </div>
      </div>
  ...
  async toggleFollow(username, btn) {
    const a = this.creators.find((x) => x.username === username);
    ...
    try {
      ...
      if (label) label.textContent = res.following ? t("profile_following", "Following") : t("creators_follow", "Follow");
      const svg = btn.querySelector("svg");
      if (svg) svg.innerHTML = res.following ? '<path d="M20 6 9 17l-5-5"/>' : '<path d="M12 5v14M5 12h14"/>';
      const card = this.main.querySelector(`.creator-card[data-username="${CSS.escape(username)}"] .creator-stats`);
      if (card) card.innerHTML = `<b>${a.public_characters}</b> ${a.public_characters === 1 ? t("creators_character_singular") : t("creators_character_plural")} &middot; <b>${a.follower_count || 0}</b> ${a.follower_count === 1 ? t("creators_follower_singular", "follower") : t("creators_follower_plural", "followers")}`;
    } catch (err) {
